...
function addSFC(coreDir) {
  fs.readdirSync(new URL(coreDir, import.meta.url).pathname, { withFileTypes: true })
    ...
    .forEach(({ name: folderName }) => {
      // table собирается из index.ts (бандлит Table + compound Column/ColumnGroup в table.mjs
      // как named-экспорты) — см. явный addEntry("table", "index.ts", "table") ниже.
      if (folderName === "table") return
