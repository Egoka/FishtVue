...
function addSFC(coreDir) {
  fs.readdirSync(new URL(coreDir, import.meta.url).pathname, { withFileTypes: true })
    ...
    .forEach(({ name: folderName }) => {
      // table/menu собираются из index.ts (бандлят Table+Column/ColumnGroup и Menu+MenuItem/MenuGroup
      // в {name}.mjs как named-экспорты) — см. явные addEntry("table"/"menu", "index.ts", ...) ниже.
      if (folderName === "table" || folderName === "menu") return
