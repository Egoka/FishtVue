...
function addSFC(coreDir) {
  fs.readdirSync(new URL(coreDir, import.meta.url).pathname, { withFileTypes: true })
    ...
    .forEach(({ name: folderName }) => {
      // table/menu/form собираются из index.ts (бандлят родителя + renderless-детей и реестр типов
      // в {name}.mjs как named-экспорты) — см. явные addEntry("table"/"menu"/"form", "index.ts", ...) ниже.
      if (folderName === "table" || folderName === "menu" || folderName === "form") return
