...
/**
 #### `isNotEmpty` Function Documentation

 The `isNotEmpty` function is a utility function that checks if a value is not empty. It takes one parameter: `value`, which is the value to be checked. It returns a boolean value indicating whether the value is not empty.
 ...
 The `isNotEmpty` function can be used to check if a value is not empty. It uses the `isEmpty` function to determine if the value is empty and negates the result using the logical NOT operator (`!`).

 Here is an example of how the `isNotEmpty` function can be used:

 ```typescript
 const obj = { name: 'John', age: 25 };
 const isNotEmptyObj = isNotEmpty(obj); // true

 const emptyArray = [];
 const isNotEmptyArray = isNotEmpty(emptyArray); // false

 const str = '';
 const isNotEmptyString = isNotEmpty(str); // false
 ```

 In this example, the `isNotEmpty` function is called with different values. It returns `true` for a non-empty object (`obj`), and `false` for an empty array (`emptyArray`) and an empty string (`str`).

 **Note**: The `isNotEmpty` function can be used with various types of values to check if they are not empty.
 */
 ...
 */
export declare function unFreeze<T>(obj: T): T
