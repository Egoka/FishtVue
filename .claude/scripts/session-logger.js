...
function readAgentContextFile(_sessionId) {
  try {
    ...
    const contextPath = path2.join(logsDir, sessionFolder, "agent-context.json");
    if (!fsSync.existsSync(contextPath)) {
      return void 0;
    }
    const content = fsSync.readFileSync(contextPath, "utf8");
...
function readAgentFromWorkflowState(_sessionId) {
  try {
    ...
    const statePath = path2.join(logsDir, sessionFolder, "workflow-state.json");
...
async function ensureSessionFolder(sessionId) {
  ...
  const sessionDir = path2.join(logsDir, folderName);
  try {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      entry.name.endsWith(`-${sessionId}`)) {
        return path2.join(logsDir, entry.name);
      }
    }
    return void 0;
  } catch {
...
function getSessionLogPath(sessionId, logType, sessionDir = void 0) {
  ...
  sessionDir ??= path2.join(logsDir, getOrCreateSessionFolder(sessionId));
  if (agentContext.isAgent) {
    ...
    const agentDir = path2.join(sessionDir, agentFolder);
    try {
    ...
    if (logType === "hook-state") {
      return path2.join(agentDir, "hook-state.jsonl");
    }
    return path2.join(agentDir, filename);
  }
  if (logType === "hook-state") {
    return path2.join(sessionDir, "hook-state.jsonl");
  }
  return path2.join(sessionDir, filename);
}
...
function writeAgentMetadata(agentDir, agentPid, agentType, toolUseId) {
  const metadataPath = path2.join(agentDir, "metadata.json");
...
async function logTokenUsage(entry, sessionId) {
  ...
  const tokensPath = path2.join(sessionDir, "tokens.jsonl");
  try {
