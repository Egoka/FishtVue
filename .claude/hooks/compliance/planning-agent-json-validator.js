...
// src/templates/.claude/hooks/compliance/planning-agent-json-validator.ts
init_session_logger();
...
function extractJSON(content) {
  ...
  if (jsonBlockMatch && jsonBlockMatch[1]) {
    try {
      ...
      return {
        valid: false,
        error: `JSON in code block is invalid: ${e instanceof Error ? e.message : String(e)}`
      };
    }
  }
  return { valid: false, error: "Content is not valid JSON and no JSON code block found" };
}
function validateFeaturePlannerSchema(json) {
  const warnings = [];
  if (typeof json !== "object" || json === null) {
    return { valid: false, warnings: ["Output is not a JSON object"] };
  }
  const output = json;
  ...
  if (!output.agent_type) {
    warnings.push("Missing 'agent_type' field");
  }
  if (!output.full_requirements) {
    warnings.push("Missing 'full_requirements' field");
  }
  if (!output.tasks || !Array.isArray(output.tasks)) {
    warnings.push("Missing or invalid 'tasks' array");
  }
  if (!output.nextAction) {
    warnings.push("Missing 'nextAction' field for orchestration");
  }
...
process.stdin.on("end", () => {
  try {
    ...
    const agentContext = detectAgentContext();
    if (!agentContext.isAgent || agentContext.agentName !== "feature-planner") {
      outputEmpty();
      exitSuccess();
      return;
    }
    const filePath = toolInput.file_path ?? "";
    if (!filePath.includes("backlog/specs/")) {
      outputEmpty();
      exitSuccess();
      return;
    }
    let content;
    try {
      content = fsSync2.readFileSync(filePath, "utf8");
    } catch (error) {
      logHookActivity("planning-agent-json-validator", "PostToolUse", {
        details: `Could not read spec file for validation: ${error instanceof Error ? error.message : String(error)}`,
        status: "warning"
      });
      outputEmpty();
      exitSuccess();
    ...
    if (!extraction.valid) {
      logHookActivity("planning-agent-json-validator", "PostToolUse", {
        details: `Feature-planner output validation failed: ${extraction.error}`,
        file: filePath,
        status: "warning"
      });
      process.stderr.write(`
[planning-agent-json-validator] WARNING: Feature spec file may not be valid JSON
`);
      process.stderr.write(`  File: ${filePath}
`);
      process.stderr.write(`  Error: ${extraction.error}
`);
      process.stderr.write(`  The orchestration layer may fail to parse this output.
...
`);
    ...
    const schemaValidation = validateFeaturePlannerSchema(extraction.json);
    if (!schemaValidation.valid) {
      logHookActivity("planning-agent-json-validator", "PostToolUse", {
        details: `Feature-planner output schema warnings: ${schemaValidation.warnings.join(", ")}`,
        file: filePath,
        status: "warning"
      });
      process.stderr.write(`
[planning-agent-json-validator] Schema validation warnings:
`);
      process.stderr.write(`  File: ${filePath}
`);
...
`);
      ...
      logHookActivity("planning-agent-json-validator", "PostToolUse", {
        details: `Feature-planner output is valid JSON with correct schema`,
        file: filePath,
        status: "success"
      });
    }
    ...
    console.error("Planning agent JSON validator hook error:", errorMessage);
    outputEmpty();
    exitSuccess();
  }
});
