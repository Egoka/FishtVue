...
// src/templates/.claude/hooks/compliance/planning-agent-tool-blocker.ts
init_session_logger();
...
// src/templates/.claude/hooks/compliance/planning-agent-tool-blocker.ts
var TOOL_BLOCK_RULES = [
  {
    name: "planning-agent-task-blocker",
    agents: ["feature-planner", "cto-architect", "v0-planner"],
    blockedTools: ["Task"],
    errorTitle: "PLANNING AGENTS CANNOT SPAWN OTHER AGENTS",
    errorHelp: "Planning agents output JSON for orchestration - they don't spawn agents directly.",
    additionalContext: [
      "\u2022 Your job is to analyze, plan, and output structured JSON",
      "\u2022 The main Claude process reads your JSON output and spawns agents",
      "\u2022 This prevents recursive agent spawning and ensures proper orchestration",
      "\u2022 Include 'nextAction' field in your JSON to signal what agents should run"
    ]
  },
  {
    name: "planning-agent-notebook-blocker",
    agents: ["feature-planner", "cto-architect", "v0-planner"],
    blockedTools: ["NotebookEdit"],
    errorTitle: "PLANNING AGENTS CANNOT EDIT NOTEBOOKS",
    errorHelp: "Planning agents define requirements, they don't modify code or notebooks.",
    additionalContext: [
      "\u2022 Your role is product/technical planning, not implementation",
      "\u2022 Notebook modifications should be done by implementation agents",
      "\u2022 Focus on outputting structured JSON with clear specifications"
    ]
  }
];
function formatBlockMessage(rule, toolName, agentName) {
  const red = "\x1B[31m";
...
`;
  output += `${white}Agent:${reset} ${cyan}${agentName}${reset}
`;
  output += `${white}Blocked tool:${reset} ${red}${toolName}${reset}

`;
  if (rule.additionalContext.length > 0) {
...
process.stdin.on("end", () => {
  try {
    ...
    const agentContext = detectAgentContext();
    if (!agentContext.isAgent || !agentContext.agentName) {
      logHookActivity("planning-agent-tool-blocker", "PreToolUse", {
        details: "Not in agent context, allowing",
        status: "success"
      });
      outputAllow();
      exitSuccess();
    ...
    const currentAgent = agentContext.agentName;
    for (const rule of TOOL_BLOCK_RULES) {
      if (!rule.agents.includes(currentAgent)) {
        continue;
      }
      if (rule.blockedTools.includes(toolName)) {
        logHookActivity("planning-agent-tool-blocker", "PreToolUse", {
          details: `Blocked ${toolName} for ${currentAgent} by rule '${rule.name}'`,
          status: "blocked"
        });
        const response = {
          hookSpecificOutput: {
            block: {
              reason: formatBlockMessage(rule, toolName, currentAgent)
            },
    ...
    logHookActivity("planning-agent-tool-blocker", "PreToolUse", {
      details: `Allowed ${toolName} for ${currentAgent}`,
      status: "success"
    });
    outputAllow();
    exitSuccess();
    ...
    console.error("Planning agent tool blocker hook error:", errorMessage);
    outputAllow();
    exitSuccess();
  }
});
