...
// src/templates/.claude/hooks/compliance/logger-posttooluse.ts
...
async function appendLog(entry) {
  try {
    ...
    await writeToSessionLog("compliance", enrichedEntry);
    const globalLogDir = path4.join(__dirname, "../../logs");
    const globalLogFile = path4.join(globalLogDir, "global-compliance.jsonl");
    ...
    console.error(
      "[PostToolUse] Failed to write compliance log:",
      errorMessage
    );
  }
}
...
function extractToolName(event) {
  return event.tool_name ?? event.tool?.name ?? event.toolName ?? "";
}
function extractToolParameters(event) {
  const params = event.tool_input ?? event.tool?.parameters ?? event.parameters ?? {};
  return params;
}
function isToolEvent(value) {
  return "tool_name" in value && typeof value.tool_name === "string" || "tool" in value && typeof value.tool === "object" || "toolName" in value && typeof value.toolName === "string";
}
async function logToolUse(toolName, _parameters, result) {
  const logEntry = {
    hasResult: result !== void 0 && result !== "",
    hook: "PostToolUse",
...
process.stdin.on("end", () => {
  void (async () => {
    try {
      ...
      if (!isToolEvent(parsedData)) {
        exitSuccess();
      }
      const event = parsedData;
      const toolName = extractToolName(event);
      const toolParameters = extractToolParameters(event);
      await logToolUse(toolName, toolParameters, void 0);
      exitSuccess();
    } catch {
      exitSuccess();
    }
