...
// src/templates/.claude/hooks/core/mcp-session-manager.ts
...
// src/templates/.claude/hooks/core/mcp-session-manager.ts
var SESSIONS_DIR = join(homedir(), ".claude", "mcp-sessions");
var SESSION_FILE = join(SESSIONS_DIR, "active-sessions.json");
var JSON_INDENT_SPACES = 2;
if (!existsSync2(SESSIONS_DIR)) {
  mkdirSync2(SESSIONS_DIR, { recursive: true });
}
function cleanupV0Sessions() {
  const sessions = getActiveSessions();
  if (sessions.sessions.v0 !== void 0) {
    delete sessions.sessions.v0;
...
function getActiveSessions() {
  try {
    ...
    const data = readFileSync2(SESSION_FILE, "utf8");
...
async function main() {
  ...
  switch (mcpTool) {
    ...
    case "exa":
    case "replicate": {
      sessions.sessions[mcpTool] ??= {
        callCount: 0,
        startTime: Date.now()
      };
      const session = sessions.sessions[mcpTool];
    ...
    default: {
      ...
      const defaultSession = sessions.sessions[mcpTool];
      defaultSession.lastUsed = Date.now();
      defaultSession.callCount = (defaultSession.callCount ?? 0) + 1;
      break;
    }
  }
  saveActiveSessions(sessions);
  const currentSession = sessions.sessions[mcpTool];
  const activityDetails = {
    callCount: currentSession?.callCount ?? 0,
    details: `MCP tool ${mcpTool} called`,
    mcpTool,
    status: "success",
    toolName
  };
  logHookActivity("mcp-session-manager", "PreToolUse", activityDetails);
  outputEmpty();
}
...
function saveActiveSessions(sessions) {
  try {
    writeFileSync(SESSION_FILE, JSON.stringify(sessions, void 0, JSON_INDENT_SPACES));
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    process.stderr.write(`[mcp-session-manager] Failed to save session state: ${errorMessage}
`);
  }
}
try {
  ...
  const hookError = error instanceof Error ? { message: error.message } : { message: String(error) };
  process.stderr.write(`[mcp-session-manager] Hook failed: ${hookError.message}
`);
  outputEmpty();
}
