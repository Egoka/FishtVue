...
// src/templates/.claude/hooks/orchestration/agent-context-cleaner.ts
init_path_resolver();
...
process.stdin.on("end", () => {
  void (async () => {
    try {
      ...
      const wasCleared = await clearAgentContext();
      if (wasCleared) {
        logHookActivity("agent-context-cleaner", "PostToolUse", {
          details: "Cleared agent context after Task completion",
          status: "success"
        });
      } else {
        logHookActivity("agent-context-cleaner", "PostToolUse", {
          details: "No agent context to clear (file not found)",
          status: "success"
        });
      }
      outputEmpty();
      ...
      console.error("Agent context cleaner hook error:", errorMessage);
      outputEmpty();
      exitSuccess();
    }
  })();
