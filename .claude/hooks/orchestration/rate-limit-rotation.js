...
// src/templates/.claude/hooks/orchestration/rate-limit-rotation.ts
...
async function processStopEvent(event) {
  const combinedText = [
    event.error ?? "",
    event.message ?? "",
    event.reason ?? ""
  ].join(" ");
  if (combinedText.trim().length === 0 || !isRateLimitStop(combinedText)) {
    outputEmpty();
    return;
  }
  logHookActivity("rate-limit-rotation", "Stop", {
    details: "Rate limit detected in main session, triggering account rotation",
    status: "info"
  });
  const result = await triggerAccountRotation();
  if (result.success) {
    logHookActivity("rate-limit-rotation", "Stop", {
      details: `Account rotated to ${result.newAccountId ?? "unknown"} after main session rate limit`,
      status: "success"
    });
    safeJsonOutput({
      hookSpecificOutput: {
        additionalContext: `[RATE LIMIT RECOVERY] Account was automatically rotated to ${result.newAccountId ?? "a new account"}. You can resume immediately - the new account credentials are active.`
      }
    });
  } else {
    logHookActivity("rate-limit-rotation", "Stop", {
      details: `Account rotation failed: ${result.error ?? "unknown"}`,
      status: "warning"
    });
    outputEmpty();
  }
...
process.stdin.on("end", () => {
  void (async () => {
    try {
      ...
      console.error("[rate-limit-rotation] Error:", error);
      outputEmpty();
      exitSuccess();
    }
  })();
