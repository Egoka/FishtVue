...
date: 18/10/2025
seo:
  title: "Компоненты FishtVue - Полная библиотека UI компонентов"
  description: "Изучите все компоненты FishtVue для Vue 3: формы, кнопки, таблицы данных, панели, оверлеи, меню, уведомления и многое другое."
  keywords:
    - компоненты fishtvue
    - vue 3 компоненты
