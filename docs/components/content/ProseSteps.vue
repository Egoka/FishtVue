...
<script setup lang="ts">
  ...
  const level = computed(() => classesLevels.value[props.level ?? 3])
  const classes = computed<Record<"base", string>>(() => ({
    base: `ms-4 border-s border-neutral-200 dark:border-neutral-800 ps-8 [counter-reset:step] ${level.value}`
  }))
</script>

<template>
  <div :class="[classes.base, props.class]">
    <slot />
  </div>
</template>
