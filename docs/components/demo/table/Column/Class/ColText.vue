<script setup lang="ts">
  ...
  const { t } = useI18n()
  const { data } = useAsyncData("tableColumnClassColText", () =>
    Promise.resolve(generateData(5, "fruits", ["name", "color", "weightGrams"]))
  )
  const columns = ref<TableProps["columns"]>([
    { dataField: "name", caption: t("caption.name"), class: { colText: "class-th-red" } },
    { dataField: "color", caption: t("caption.color"), class: { colText: "class-th-orange" } },
    { dataField: "weightGrams", caption: t("caption.weightGrams"), class: { colText: "class-th-green" } }
  ])
</script>
