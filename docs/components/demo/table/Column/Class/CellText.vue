<script setup lang="ts">
  ...
  const { t } = useI18n()
  const { data } = useAsyncData("tableColumnClassCellText", () =>
    Promise.resolve(generateData(5, "fruits", ["name", "color", "weightGrams"]))
  )
  const columns = ref<TableProps["columns"]>([
    { dataField: "name", caption: t("caption.name"), class: { cellText: "class-th-red" } },
    { dataField: "color", caption: t("caption.color"), class: { cellText: "class-th-orange" } },
    { dataField: "weightGrams", caption: t("caption.weightGrams"), class: { cellText: "class-th-green" } }
  ])
</script>
