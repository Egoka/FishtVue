<script setup lang="ts">
  ...
  const { t } = useI18n()
  const { data } = useAsyncData("tableColumnClassTh", () =>
    Promise.resolve(generateData(5, "fruits", ["name", "color", "weightGrams"]))
  )
  const columns = ref<TableProps["columns"]>([
    { dataField: "name", caption: t("caption.name"), class: { th: "class-th-red" } },
    { dataField: "color", caption: t("caption.color"), class: { th: "class-th-orange" } },
    { dataField: "weightGrams", caption: t("caption.weightGrams"), class: { th: "class-th-green" } }
  ])
</script>
