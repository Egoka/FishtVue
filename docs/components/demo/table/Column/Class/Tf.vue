<script setup lang="ts">
  ...
  const { t } = useI18n()
  const { data } = useAsyncData("tableColumnClassTf", () =>
    Promise.resolve(generateData(5, "fruits", ["name", "color", "weightGrams"]))
  )
  const columns = ref<TableProps["columns"]>([
    { dataField: "name", caption: t("caption.name"), class: { tf: "class-th-red" } },
    { dataField: "color", caption: t("caption.color"), class: { tf: "class-th-orange" } },
    { dataField: "weightGrams", caption: t("caption.weightGrams"), class: { tf: "class-th-green" } }
  ])
</script>
