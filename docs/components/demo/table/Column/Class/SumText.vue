<script setup lang="ts">
  ...
  const { t } = useI18n()
  const { data } = useAsyncData("tableColumnClassSumText", () =>
    Promise.resolve(generateData(5, "fruits", ["name", "color", "weightGrams"]))
  )
  const columns = ref<TableProps["columns"]>([
    { dataField: "name", caption: t("caption.name"), class: { sumText: "class-th-red" } },
    { dataField: "color", caption: t("caption.color"), class: { sumText: "class-th-orange" } },
    { dataField: "weightGrams", caption: t("caption.weightGrams"), class: { sumText: "class-th-green" } }
  ])
</script>

<template>
  <Table :data-source="data as any[]" summary :columns="columns"> </Table>
</template>
<style scoped>
  :deep(.class-th-red) {
    margin: 10px;
