  ...
  const structure = ref<FormProps["structure"]>([
    {
      ...
      class: "structure-first-class rounded-sm border border-gray-200 dark:border-gray-700 m-1",
    ...
    },
    {
      ...
      class: "structure-second-class rounded-sm border border-gray-200 dark:border-gray-700 m-1",
      isHidden: true,
...
<template>
  <Form ref="formTest" :structure="structure" class="w-72" v-model:formFields="formValues">
    <template #itemTitle="{ structure }">
      <h2 v-if="structure?.title?.length" class="text-xl font-semibold leading-7 text-gray-900 dark:text-gray-100">
        {{ structure?.title }}
      </h2>
      <p v-if="structure?.description?.length" class="mt-1 text-sm leading-6 text-gray-600 dark:text-gray-400">
