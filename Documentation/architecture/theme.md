...
│   ├── Harmony.ts
│   └── Sapphire.ts
├── useDarkMode.ts         # composable: реактивный флаг тёмной темы (darkModeSelector → DOM, иначе media)
├── helpers/
...
- **transforms — modern CSS properties** (v4-подход, с 2026-07-02): `translate-*`/`rotate-*`/`scale-*` эмитят независимые свойства `translate:`/`rotate:`/`scale:`; только осевые формы живут в `transform:`; `transition`/`transition-transform` покрывают `transform, translate, scale, rotate`. Комбинации вида `-translate-y-1/2 rotate-45` компонуются без двойного сдвига.

**Волна 3 (2026-09-06) — паритет с v4 по вариантам и утилитам.** Всё перечисленное ниже раньше было «вне диалекта»:

- **container queries:** утилита `@container` (`@container-normal`, `@container/{name}`) и варианты `@sm:` / `@max-md:` / `@min-[475px]:` / `@sm/main:`. До этой волны `@sm:` был не отказом, а **ложным срабатыванием**: символ `@` не участвовал в разборе, `sm` матчился обычным breakpoint'ом, и класс давал viewport-`@media` — то есть «почти работал» на full-width контейнерах и врал на остальных;
- **варианты:** `not-*:` (псевдо-класс → `:not()`, media → `@media not …`, `supports-[…]` → `@supports not (…)`), `in-*:` (неявная группа), `nth-*:` / `nth-last-*:` / `nth-of-type-*:` (в т.ч. `nth-[3n+1]:`), композиции `group-/peer- × aria-/data-`, `**:` (все потомки — раньше давал `> *`, то есть то же, что `*:`), `starting:`, именованный `supports-<feature>:`, arbitrary at-rule `[@media(…)]:`;
- **important-модификатор** в обеих формах — префиксной `!mt-4` (v3) и суффиксной `mt-4!` (v4). Раньше `!` просто экранировался в селектор, а объявление уходило в CSS без `!important`;
- **утилиты:** 3D-трансформы (`rotate-x/y/z-*`, `translate-z-*`, `scale-z-*`, унифицированный `skew-<n>`, `perspective-*`, `perspective-origin-*`, `transform-3d/flat/gpu/cpu/none`, `backface-*`), v4 gradient API (`bg-linear-to-*`, `bg-linear-<angle>`, `bg-radial`, `bg-conic`, `bg-size-*`, `bg-position-*` + модификатор интерполяции `/oklch`, `/longer`), `text-shadow-*`, `inset-shadow-*`, `inset-ring-*`, `filter-none`/`backdrop-filter-none`, цветной `drop-shadow-<color>`, `wrap-anywhere`/`wrap-break-word`/`wrap-normal`, `font-stretch-*`, `scheme-*`, `field-sizing-*`, `forced-color-adjust-*`, `transition-discrete`/`transition-normal`.

Два следствия волны 3, которые стоит знать:

- `--fv-drop-shadow-color` и `--fv-text-shadow-color` вынесены в переменные с fallback'ом на прежний литерал. Внешний вид без класса цвета не изменился, но `drop-shadow-lg drop-shadow-red-500` теперь работает в любом порядке — классы пишут разные свойства;
- `inset-shadow-*` / `inset-ring-*` используют **пятислотовую** цепочку `box-shadow` (inset-слои первыми, как в v4), тогда как прежние `shadow-*` / `ring-*` сохраняют свою трёхслотовую. Переписывать вторую значило бы изменить вывод у всех существующих классов, поэтому при комбинации inset-класс должен идти в строке классов **после** `shadow-*`/`ring-*`.
...
**Вне диалекта** (fail-closed + dev-warn) — после волны 3 остался **один** пункт: семейство `mask-*`. Решение осознанное и зафиксировано планом [issues/uno-engine.md Issue 4](../issues/uno-engine.md) как приоритет 3. Причина не в объёме словаря, а в том, что маски-градиенты v4 — это не отображение «класс → свойство», а подсистема из двух десятков связанных custom properties (`--tw-mask-linear`, посторонние `--tw-mask-{side}-{from,to}-{color,position}`, композиция через `mask-composite: intersect`) с собственными дефолтами в базовом слое. Воспроизвести её «на глаз» означало бы зашить в движок правдоподобную, но непроверяемую модель: тесты подтверждали бы мою реконструкцию, а не поведение Tailwind. Чистая граница с dev-warn честнее полуподдержки.

Всё остальное из прежнего списка «вне диалекта» реализовано волной 3 — см. врезку выше.
...
| `tailwind(class, options?)`  | `(class: string, options?: { selector?, darkSelector? }) => string` | Конвертирует один Tailwind-подобный класс в CSS-сниппет. **Fail-closed** (2026-07-02): нераспознанный variant-префикс, «чужое» семейство (`mask-*`) или пустое/`undefined`-значение → `undefined` + dev-only `console.warn` `[FishtVue tailwind] class "…" was dropped` (гейт `NODE_ENV !== "production"`, дедуп по классу). Невалидный CSS в `<style>`-теги не попадает; контракт — [unoStyle/failClosed.test.ts](../../lib/theme/unoStyle/failClosed.test.ts), [issues/uno-engine.md](../issues/uno-engine.md) Issue 1/3. Границы диалекта (v3.4 + v4-расширения, вкл. arbitrary properties и modern transforms) — §3.1. |
...
| `useDarkMode()`              | `() => Ref<boolean>`                                                | Реактивный флаг тёмной темы, согласованный с движком: читает DOM по `optionsTheme.darkModeSelector`, иначе — `prefers-color-scheme: dark`. Слушатели снимаются в `onUnmounted`, поэтому только из `setup()`; на сервере всегда `false`. Введён 2026-09-05 — до этого логика была скопирована в Table и Calendar, а TextEditor её не имел вовсе. |
...
- `updateSurfacePalette` пишет `semantic.surface` и эмитит `--fv-surface-{tone}`; с 2026-07-04 `surface` — 23-й именованный цвет в `primitive.ts` (дефолт — копия `gray`), и первый батч из 11 компонентов (Menu, Accordion, Icons, Select, TextEditor, Separator, Calendar, Input, Form, Label, Textarea) потребляет его вместо хардкода `gray-*`/`stone-*`/`neutral-*`/`slate-*`/`zinc-*`. Residual — 8 компонентов (Badge, Switch, Split, Pagination, Table, InputLayout, FixWindow, Dialog), закрывшие свой номерной B10 через `forced-colors`+theme-accent, но оставившие структурные нейтрали, и цвета-интенты Alert (success/warning/info/error, отдельный semantic-slot epic) — см. [issues/README.md](../issues/README.md).
