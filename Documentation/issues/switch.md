...
> **2026-08-02 — регрессия `unstyled` у нативных контролов (L53), матрица не менялась.** `Component.setStyle` под `config.unstyled` возвращает `""`, а UA-ресеты в `lib/config/baseStyle.ts` (`button.fv` / `input.fv` → `margin`/`padding`/`font: inherit`, `background-color: transparent`, `.fv` → `border-width: 0`) навешены на класс `fv`; preflight инжектится и в unstyled (гард стоит только на выдаче классов). Поэтому `<button role="switch">` и `<input type="checkbox">` — оба берут класс из одного computed `classSwitch` — оставались без единого класса и получали браузерный chrome. **Фолбэк вынесен в отдельный template-only computed `classSwitchElement = classSwitch || "fv"`**, на который переведены оба `:class`-биндинга. Экспонируемый `classSwitch` намеренно оставлен прежним (`""` под unstyled): он входит в `defineExpose` и типизирован в `SwitchExpose` (`Switch.d.ts`), то есть его значение — публичный контракт, и менять его ради CSS-хука нельзя. В styled-режиме фолбэк доказуемо недостижим, expose и рендеримый класс совпадают. `lib/config/baseStyle.ts` и `Switch.d.ts` **не** правились.
...
- **Resolution (2026-07-05, Wave 9 residual — полное закрытие):** после появления semantic-токена `surface` (23-й именованный цвет движка, дефолт — копия `gray`-шкалы; [primitive.ts:305](../../lib/theme/primitive.ts#L305), `ColorName` union в [Theme.d.ts:178](../../lib/theme/Theme.d.ts#L178)) все оставшиеся структурные нейтрали Switch мигрированы **family rename** (та же числовая тональность, без изменения самих значений):
