...
> ⚠️ **Снимок частично отработан в тот же день.** По этому плану 2026-09-05 выполнены W0 целиком и часть W2; журнал исполнения — [closure-progress.md](./closure-progress.md). Что изменилось относительно текста ниже:
>
> - **§3 поз. 11 признана ошибочной и снята** — см. пометку в таблице.
> - **Закрыты:** B13 (doc-sync), B17 (tooling), B15 + B16 (Nuxt), B3 (native form submit), B2 (Label), B4 (`arrayHandler.sort`), B1 (свёлся к doc-sync), часть B12 (i18n TextEditor). Сверх плана — **разблокирована тест-суита TextEditor** (0% → 95.89%), что закрывает самую дорогую часть эпика C2.
> - **§2.1 неактуален** — пять несогласованных чисел сведены в одно, матрица README пересчитана (`0 / 23 / 30 / 16` → `0 / 5 / 21 / 18`).
> - **Оценки §6–7 сдвинулись:** C2 переоценён с 4–7 д на 1.5–3 д, риск с высокого на низкий. Это меняет критический путь — C2 был его самым длинным звеном.
> - **Новые находки, которых нет в §4:** N8–N14, см. [closure-progress.md](./closure-progress.md).
...
### 2.1. Что заявляло документация — пять несогласованных чисел

> ✅ **Устранено 2026-09-05** (батч B13). Матрица README пересчитана по фактическим «Сводка»-таблицам, текстовые счётчики приведены к ней же. Раздел оставлен как описание того, что было найдено.
...
| − дедуп cross-cutting «print / motion / RTL / colors» (Textarea 11, Calendar 10, TextEditor 11, Theme 9 → это B5/B6)                      | −4     |
| **≈ дискретных задач**                                                                                                                | **≈30** |
...
| Targets с реально открытым кодом                                             | **12**: Calendar, TextEditor, Theme, Uno engine, Locale, Nuxt module, Label, Textarea, Select, Loading, Alert, Utilities |
...
| 5   | Wave 10.4 `Textarea.d.ts:97` — `change:modelValue` type bug ([README.md:443](./README.md#L443))                                                         | **Уже `payload: string`**                                                                                                                                                                  | [Textarea.d.ts:125](../../lib/textarea/Textarea.d.ts#L125), JSDoc «Fixed in 2026-05-11»                                                                                                                                                                                                                                                                 |
...
| ~~11~~ | ~~Wave 11 TextEditor — «разблокировать 17 skipped tests (`vi.mock`)»~~ | ❌ **ПОЗИЦИЯ ОШИБОЧНА, снята 2026-09-05** | Утверждение «0 `it.skip`, 15 живых тестов, `vi.mock` отсутствует» неверно: блок был под `describe.todo`, тестов ровно **17** — счёт roadmap был точен, — а предложенный им `vi.mock("@vueup/vue-quill")` оказался **правильным решением** и сработал. Ошибка возникла из-за того, что `describe.todo` не даёт `it.skip` в исходнике. Пункт roadmap закрыт по существу: 27/27 тестов, coverage 0% → 95.89%. Урок: считать todo/skip по отчёту vitest, а не по grep'у исходника |
...
| 17  | Wave 12 codemod «`change:modelValue` type для Textarea/TextEditor» ([README.md:508](./README.md#L508))                                                  | **Нечего кодмодить** — исправление type-only, runtime всегда эмитил string                                                                                                                 | позиции 5–6                                                                                                                                                                                                                                                                                                                                 |
| 18  | 9 per-component issues «SSR styles + sideEffects + unstyled» — все ссылаются на button.md Issues 1/8/9/14: [textarea.md:77](./textarea.md#L77), [calendar.md:156](./calendar.md#L156), [label.md:58](./label.md#L58), [theme.md:172](./theme.md#L172), [locale.md:108](./locale.md#L108), [nuxt-module.md:138](./nuxt-module.md#L138), [texteditor.md:159](./texteditor.md#L159), [alert.md:132](./alert.md#L132) (partial), [separator.md:25](./separator.md#L25) | **Все четыре корневых issue закрыты**: [button.md:34](./button.md#L34) Issue 1 ✅ 2026-06-07, [:281](./button.md#L281) Issue 8 ✅ 2026-06-07, [:306](./button.md#L306) Issue 9 ✅ 2026-06-11, [:484](./button.md#L484) Issue 14 ✅ 2026-05-11 | Wave 2 ✅ closed 2026-06-21, Wave 3.1 ✅. Это **до 8 High-счётчиков** в матрице (Calendar, Label, Textarea, Theme, Locale, Nuxt, TextEditor — по одному A2/A4-5/C17), которые снимаются doc-sync'ом |
...
| B15 | **N1**: добавить `MenuItem`/`MenuGroup`/`AccordionItem` в `FISHT_VUE_SUBCOMPONENTS` + тест регистрации                                                                                                                                                                                                   | [nuxt.ts:108-118](../../lib/module/nuxt.ts#L108-L118)                                                                  | 0.25–0.5 д |
...
| C2  | **TextEditor.** ~~0% coverage~~ ✅ 2026-09-05 (95.89%), ~~N4 хардкод-строки~~ ✅, ~~N5 dark-mode~~ ✅. **Остаток:** Quill toolbar i18n (Issues 8, 9 — лейблы `font`/`align` самого Quill) + `image-upload-request` handler вместо base64-инжекции ([TextEditor.vue](../../lib/texteditor/TextEditor.vue), Issue 6)                                                                              | ~~4–7 д~~ → **1.5–3 д** | ~~высокий~~ → **низкий** — mock-харнесс готов, Quill больше не поднимается в тестах |
...
| ID  | Что                                                                                                                                                                                        | Статус                                                                                                                                             |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| D3  | **`Aria` → `Textarea`** — rename компонента + import-update                                                                                                                                | [textarea.md:101](./textarea.md#L101) Issue 6 «open (deferred to breaking-change PR)». Без этого `textarea.md` (бывший `aria.md`) не закрывается — **сводку нельзя закрыть без major-релиза** |
...
> **Пересчёт 2026-09-05.** W0 выполнена, C2 ужался до 1.5–3 д (его самая дорогая часть — coverage при заблокированной суите — закрыта). Критический путь стал:
>
> ```
> W1 (решения) → C2 остаток (1.5–3 д) → W5 (6.5–12 д)
> ≈ 8–15 инженеро-дней ≈ 2–3 недели
> ```
>
> Узкое место сместилось с TextEditor на **W5** (migration-guide + breaking changes + major-релиз), а он целиком упирается в решение D3 — когда планируется major. Пока решения нет, длину проекта задаёт не инженерия, а этот выбор.
...
**Закрытие сводки требует major-релиза** — из-за D3 (`Aria` → `Textarea`). Если major в ближайшие месяцы не планируется, [textarea.md](./textarea.md) не закроется и состояние «сводка закрыта» недостижимо по текущему определению.
