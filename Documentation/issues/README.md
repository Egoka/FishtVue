...
- [ ] **W2 — семейство InputLayout:** `InputLayoutClassKey` (`base`/`label`/`help`/`message`/`before`/`after`/`animation`), `control`; `hasValue`/`invalid`/`clearable`; Select → `SelectItem`, `options`, `searchable`, `emptyText`, `fixWindowProps`; Calendar `datePickerProps`, `closeOnSelect`, `ready`; TextEditor `dialogProps`/`editorProps`; `data-{name}` на корне, `-control` на контроле.
