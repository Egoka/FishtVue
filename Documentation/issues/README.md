...
- [ ] **W5 — контейнеры:** Menu (`classes` + `width`/`height`, `orientation`, `firstLetter`, `fixWindowProps`, `item-*` emits, `Menu*Data`), Split (`classes.panel/separator`, `orientation`, `separatorFade`, `PanelSize`), Form (`mode`, `labelMode`, `classes.section/grid/field/footer`, `hidden`, `typeComponent`, `FieldTextarea`, `v-model:invalid`).
- [ ] **W6 — emits-guard:** `emitsNaming.test.ts` без `PENDING`, §6 всех доков, dev-patterns §7 финал.
