...
| **TOTAL**       | **27 active + 3 done**                     | **0**    | **0** | **1**  | **0**  |
...
Аудит-документов — **27 активных** в `issues/` плюс **3** в [./done/](./done/) (Dialog ✅ 2026-05-11, FixWindow ✅ 2026-05-16, Config ✅ 2026-05-20). Открытых позиций по матрице — **1** (0 Critical / 0 High / 1 Medium / 0 Low) — переименование `Aria` → `Textarea`, которое по решению R6 выполняется в финальном major вместе с остальными breaking changes; плюс 6 позиций Calendar и TextEditor в разделе «Заморожено до переписывания», которые в TOTAL не идут. Решённые остаются в файлах зачёркнутыми. Прогресс закрытия отслеживается через чекбоксы в Fix roadmap ниже и зачёркнутые блоки внутри каждого `<component>.md`.
