...
- [ ] **W1 — листья:** Icons (база на `<i data-icon>`, `classes.icon`), Loading, Label (`class`↔`classes.text`, `title→label`, `type→labelMode`, `isRequired→required`, `animate→animated`), typo `4-dots-gooey`.
...
- [ ] **W3a:** Button/Badge `mode→variant`, Separator `orientation` + `segment*/line*` + `data-separator-start/end`, Switch, Accordion (`items`, `toggle` payload, `AccordionItemData`).
