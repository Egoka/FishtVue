...
| --------------- | ------------------------------------------ | -------- | ------ | ------ | ------ |
| Button          | [button.md](./button.md)                   | 0        | 4      | 6      | 4      |
...
| Table           | [table.md](./table.md)                     | 0        | 1      | 2      | 1      |
...
| Split           | [split.md](./split.md)                     | 0        | 1      | 2      | 1      |
...
| **TOTAL**       | **27 active + 3 done**                     | **2**    | **81** | **54** | **46** |

Всего **183 active issues** распределены по 27 активным документам аудита (Dialog ✅ 2026-05-11 + FixWindow ✅ 2026-05-16 + Config ✅ 2026-05-20 → перенесены в `./done/`). Прогресс закрытия отслеживается через чекбоксы в Fix roadmap ниже и зачёркнутые блоки внутри каждого `<component>.md`.
...
- [ ] [lib/package.json:17-18](../../lib/package.json#L17) — добавить root `exports` map с условиями `types`/`import`/`default` для всех subpath: `./button`, `./input`, `./theme`, `./config`, `./component`, `./locale`, `./module`, `./plugins/nuxt`, `./utils/*`, `./types` · [button.md Issue 9](./button.md) · **ОТЛОЖЕНО (build-verified заход):** root `exports` отключает folder-main резолв nested package.json + требует enumerate всех self-referential `fishtvue/X/Y.mjs` — неверная карта ломает резолв у всех потребителей; нужен `npm pack` + smoke `npm install` на `node16`/`bundler`. ESM-only ратифицирован ✅ 2026-06-07 (см. ниже).
