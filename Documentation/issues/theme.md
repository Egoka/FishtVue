...
>
> Первый батч из 11 компонентов (Menu, Accordion, Icons, Select, TextEditor, Separator, Calendar, Input, Form, Label, Textarea) мигрирован на `surface-*` вместо хардкода `gray-*`/`stone-*`/`neutral-*`/`slate-*`/`zinc-*` — см. соответствующие `Documentation/issues/<name>.md`.
>
