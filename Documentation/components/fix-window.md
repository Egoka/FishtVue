...
- **Lifecycle:** `Component.__hooks()` ([component/index.ts:79–84](../../lib/component/index.ts#L79-L84)) инжектит стили на `onServerPrefetch + vueOnMounted`; `onMounted` подписывается на trigger-event (`addOpenListener`); `onBeforeUnmount` снимает все listeners + tears down click-outside / Escape / Floating UI's autoUpdate.
- **Поток позиционирования:** [`useFloating(referenceRef, fixWindow, { placement, strategy, middleware: [offset, flip, shift], whileElementsMounted: autoUpdate })`](https://floating-ui.com/docs/useFloating) рассчитывает координаты автоматически. FishtVue `position` мапится в Floating UI `Placement` через `positionToPlacement(...)`: `top-left → top-start`, `top-right → top-end`, …, `right-top → right-start` и т.д. — `start`/`end` обеспечивает корректное RTL-зеркалирование на documents с `dir="rtl"`.
- **byCursor:** virtual reference element создаётся в `virtualReferenceEl` computed на основе `positionMouse: { x, y }` из MouseEvent — Floating UI работает с виртуальной точкой как с рефом.
...
| `marginPx`            | `number \| 2 \| 5 \| 10`                               | `10`                                          | Margin между popover и trigger (Floating UI `offset`).                                                                                                                                                                                                         |
| `translatePx`         | `number \| 2 \| 5 \| 10`                               | `0`                                           | Fine-tune смещения (суммируется с `marginPx` для offset).                                                                                                                                                                                                      |
