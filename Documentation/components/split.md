...
- **Lifecycle:** `Component.__hooks()` сам регистрирует `onServerPrefetch + vueOnMounted → initStyle()` — в SFC нет ручного `Split.initStyle()`. `onMounted` инициализирует `ResizeObserver` для отслеживания смены размеров контейнера и восстанавливает сохранённые размеры ([Split.vue:167-176](../../lib/split/Split.vue#L167-L176)).
...
- **Стили:** через `setStyle`. Курсор во время drag задаётся overlay-элементом `<div data-split-drag-overlay class="fixed inset-0">` ([Split.vue:104-106](../../lib/split/Split.vue#L104-L106), [Split.vue:762](../../lib/split/Split.vue#L762)) — `document.body.classList` **не** мутируется (несколько Split на странице не конфликтуют).
...
- Resize handle — `<div role="separator" tabindex="0">` с `aria-orientation` (= `orientation`), `aria-valuenow`/`aria-valuemin`/`aria-valuemax` (по размеру и `min`/`max` панели) и `aria-controls`, ссылающимся на `id` управляемой панели ([Split.vue:704-720](../../lib/split/Split.vue#L704-L720)). Disabled-разделитель помечается `aria-disabled="true"`.
...
- **RTL:** для horizontal orientation resize работает в обе стороны — `isRtlHorizontal()` ([Split.vue:408](../../lib/split/Split.vue#L408)) детектит `getComputedStyle(...).direction === "rtl"` и зеркалит pointer-математику и стрелки. Физических `left/right` CSS-offset'ов у Split нет, поэтому логические-классы не требуются.
