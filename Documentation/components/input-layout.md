---
title: InputLayout
summary: Контейнер-обёртка для form-controls — label, error, help, clear/copy кнопки. С 1.0.0: `class` — корень, внутренние элементы — карта `classes` (`base`/`label`/`help`/`message`/`before`/`after` + aspect `animation`), positive-булевы `hasValue`/`invalid`/`clearable`.
updated: 2026-09-14
stability: stable
since: 0.2.11
---
...
lib/inputlayout/
├── InputLayout.vue
├── InputLayout.d.ts        # 382 строки
├── InputLayout.test.ts     # 82 кейса
└── package.json
```
...
- **Поток данных:** props → computed states → slot rendering. `value`/`hasValue` управляют состоянием floating-label.
- **Стили:** `InputLayout.resolveClasses<InputLayoutClassKey>(props)` — `cls(key, …base)` для собственных элементов, `raw("label")` для hand-off'а в `Label`, `pick("animation", …)` для aspect-ключа (dev-patterns §2 B–D, §4).
...
- **Print / high-contrast:** style-for-print (`print:border print:bg-white print:text-black print:shadow-none` на корне — печатается монохромным, не `display:none`); `forced-colors:outline` на рамке поля (`classes.base`) — граница видима в Windows high-contrast.
- **Unstyled:** при `app.use(FishtVue, { unstyled: true })` на корне и внутренних элементах остаются только `fv` и классы потребителя (`class` / `classes.<key>`) — база, mode и `fishtvue-input-layout` срезаются (dev-patterns §2 E).
...
<template>
  <InputLayout :value="text" label="Custom field" :has-value="!!text" clearable @clear="text = ''">
    <input v-model="text" />
  </InputLayout>
</template>
```
...
`InputLayoutProps` ([InputLayout.d.ts:43-163](../../lib/inputlayout/InputLayout.d.ts#L43-L163)):

| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `value` | `any` | — | Текущее значение (для floating-label и clear-кнопки). **Обязателен**. |
| `hasValue` | `boolean` | `false` | Есть ли у контрола значение — управляет состоянием лейбла (`labelType`). Form-controls вычисляют сами. |
| `mode` | `StyleMode` (`"filled" \| "outlined" \| "underlined"`) | `componentsStyle` → `"outlined"` | Визуальный режим. |
| `id` | `string` | (auto `useId()`) | Id slotted-контрола. Прокидывается в default-слот (`scope.id`) и связывает `<label for>` / `aria-labelledby` (см. §12 A11y). |
| `label` | `string` | `""` | Текст label. |
| `labelMode` | `LabelMode` | `"offsetDynamic"` | Режим label (`dynamic`/`static`/…). |
| `invalid` | `boolean` | `false` | Состояние ошибки (игнорируется при `disabled`). |
| `messageInvalid` | `string` | `""` | Сообщение ошибки. |
| `required` | `boolean` | `false` | Required-маркер (звёздочка у лейбла). |
| `loading` | `boolean` | `false` | Loading-индикатор. |
| `disabled` | `boolean` | `false` | Disabled. |
| `help` | `string` | `""` | Help-text (иконка с tooltip). |
| `clearable` | `boolean` | `false` | Показать clear-кнопку при непустом `value` (emit `clear`). |
| `width` / `height` | `TWidth` / `THeight` | — | Размеры. |
| `class` | `StyleClass` | — | Классы корня `<div data-input-layout>` (dev-patterns §2 A). |
| `classes` | `ClassesMap<InputLayoutClassKey>` | — | Карта классов внутренних элементов — см. §5.1. |
| `offsetTop` | `number \| string \| (() => number)` | `0` | Вертикальный offset для `scroll-margin-top` invalid-региона (sticky-header awareness). |

### 5.1 Classes keys

`InputLayoutClassKey` ([InputLayout.d.ts:38](../../lib/inputlayout/InputLayout.d.ts#L38)) — ключи карты `classes`; `root` добавляется `ClassesMap` и эквивалентен prop'у `class`.

| Key | Element (`data-*`) | Kind | Default |
| --- | --- | --- | --- |
| `root` | `[data-input-layout]` | element | `relative rounded-md` + фон режима + print-стили |
| `base` | `[data-input-layout-base]` | element | рамка поля: `rounded-md w-full …` + mode + state |
| `label` | `[data-label]` (корень `Label`) | element | — (hand-off в `Label.class`) |
| `help` | `[data-input-layout-help]` | element | `relative mr-2` |
| `message` | `[data-input-layout-message-invalid]` | element | `absolute block text-red-600 … invisible` |
| `before` | `[data-input-layout-before]` | element | `absolute inset-y-0 left-0 flex items-center pr-1` |
| `after` | `[data-input-layout-after]` | element | `flex pr-2` |
| `animation` | `[data-input-layout]` + `[data-input-layout-base]` | **aspect** | `motion-safe:transition-all motion-safe:duration-550` (`""` отключает) |

**element** — аддитивные ключи: база → `options.classes[key]` → `props.classes[key]` через twMerge.
**aspect** — заменяющие: `props ?? options ?? default`, пустая строка отключает целиком (dev-patterns §2 B).
...
| `value`, `isValue`, `mode`, `label`, `labelMode`, `labelType`, `isRequired`, `isLoading`, `isDisabled`, `isInvalid`, `isClearable`, `messageInvalid`, `help`, `width`, `height`, `animation` | derived | Резолвнутые props. |
| `classBase` | `string` | Итоговый класс корня (база + `class`/`classes.root`). |
| `classLayout` | `string` | Итоговый класс рамки поля (база + mode + state + `classes.base`). |
...
<template>
  <InputLayout :value="value" :has-value="!!value" label="Custom" mode="outlined" clearable @clear="value = ''">
...
```vue
<InputLayout :value="value" :has-value="!!value" invalid message-invalid="This field is required" label="Email">
...
app.use(FishtVue, {
  componentsOptions: {
    InputLayout: {
      ...
      labelMode: "dynamic",
      clearable: true,
      // внутренние элементы — только через карту `classes`
      classes: { base: "rounded-lg", message: "text-xs", animation: "motion-safe:transition-all duration-300" }
    }
  }
})
```
...
`InputLayoutOption = Pick<InputLayoutProps, "mode" | "labelMode" | "clearable" | "width" | "height" | "class" | "classes" | "offsetTop">` ([InputLayout.d.ts:369-372](../../lib/inputlayout/InputLayout.d.ts#L369-L372)).

Слияние global + local идёт **по ключу** карты `classes` (dev-patterns §2 C): неконфликтующие классы складываются, при конфликте побеждает prop (twMerge, потребитель последний).
...
`InputLayoutProps` — родитель для всех form-controls. См. [Input](./input.md), [Select](./select.md), [Calendar](./calendar.md), [TextEditor](./text-editor.md): они расширяют `Omit<InputLayoutProps, "value" | "hasValue" | "classes">` и объявляют собственную карту `classes` поверх `InputLayoutClassKey`.

## 14. Compatibility & Stability

- **Vue:** `^3.5.x`.
- **Stability flag:** `stable` — 82 кейса.
- **Breaking changes (1.0.0, редизайн props):**
  - `class` переехал с рамки поля на **корень**; прежнее содержимое `class` теперь `classes.base`, прежний `classBody` — это `class`.
  - prop `animation` снят — это aspect-ключ `classes.animation`.
  - булевы переименованы: `isValue` → `hasValue`, `isInvalid` → `invalid`, `clear` → `clearable`.
  - `componentsOptions.InputLayout.width` действительно читается (раньше в резолвере ширины стоял `options.height`).
...
describe("InputLayout", () => {
  it("emits clear", async () => {
    const wrapper = mount(InputLayout, {
      props: { value: "x", hasValue: true, clearable: true },
...
Реальные тесты — [InputLayout.test.ts](../../lib/inputlayout/InputLayout.test.ts) (82 кейса, включая блок «Props 1.0 — class / classes / positive-булевы»).

## 16. Troubleshooting / FAQ

| Проблема                                                   | Причина                                                                                      | Решение                                                                                                                                                                                           |
| ---------------------------------------------------------- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Floating label не двигается                                | `value` не обновляется или `hasValue: false`.                                                 | Передавай `:has-value="!!value"`.                                                                                                                                                                  |
...
- ~~`animation: "transition-all duration-500" | "transition-none" | string` — open union с literals.~~ ✅ resolved 2026-09-14 — prop снят, это aspect-ключ `classes.animation`.
- ~~`classBody: StyleClass | "mb-6 rounded-md"` — литерал среди StyleClass.~~ ✅ resolved 2026-09-14 — `classBody` снят: корень адресуется prop'ом `class`.
...
- При `clearable: true` без `default-slot` clear-кнопка появляется, но не имеет к чему привязаться.
